...
func (tq *TenantQuery) QueryAliases() *AliasQuery {
	query := (&AliasClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TenantQuery) QueryWebDomains() *WebDomainQuery {
	query := (&WebDomainClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TenantQuery) QueryDNSRecords() *DNSRecordQuery {
	query := (&DNSRecordClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TenantQuery) QueryDNSZoneProviders() *DNSZoneProviderQuery {
	query := (&DNSZoneProviderClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TenantQuery) Clone() *TenantQuery {
	...
	return &TenantQuery{
		...
		withAliases:          tq.withAliases.Clone(),
		withWebDomains:       tq.withWebDomains.Clone(),
		withDNSRecords:       tq.withDNSRecords.Clone(),
		withDNSZoneProviders: tq.withDNSZoneProviders.Clone(),
...
func (tq *TenantQuery) WithAliases(opts ...func(*AliasQuery)) *TenantQuery {
	query := (&AliasClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withAliases = query
	return tq
}
...
func (tq *TenantQuery) WithWebDomains(opts ...func(*WebDomainQuery)) *TenantQuery {
	query := (&WebDomainClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withWebDomains = query
	return tq
}
...
func (tq *TenantQuery) WithDNSRecords(opts ...func(*DNSRecordQuery)) *TenantQuery {
	query := (&DNSRecordClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withDNSRecords = query
	return tq
}
...
func (tq *TenantQuery) WithDNSZoneProviders(opts ...func(*DNSZoneProviderQuery)) *TenantQuery {
	query := (&DNSZoneProviderClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withDNSZoneProviders = query
	return tq
}
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	var (
		...
		loadedTypes = [5]bool{
			tq.withUsers != nil,
			tq.withAliases != nil,
			tq.withWebDomains != nil,
			tq.withDNSRecords != nil,
			tq.withDNSZoneProviders != nil,
		}
	)
	...
	if query := tq.withAliases; query != nil {
		if err := tq.loadAliases(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Aliases = []*Alias{} },
			func(n *Tenant, e *Alias) { n.Edges.Aliases = append(n.Edges.Aliases, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withWebDomains; query != nil {
		if err := tq.loadWebDomains(ctx, query, nodes,
	...
	if query := tq.withDNSRecords; query != nil {
		if err := tq.loadDNSRecords(ctx, query, nodes,
	...
	if query := tq.withDNSZoneProviders; query != nil {
		if err := tq.loadDNSZoneProviders(ctx, query, nodes,
...
func (tq *TenantQuery) loadAliases(ctx context.Context, query *AliasQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Alias)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (tq *TenantQuery) loadWebDomains(ctx context.Context, query *WebDomainQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *WebDomain)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (tq *TenantQuery) loadDNSRecords(ctx context.Context, query *DNSRecordQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *DNSRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (tq *TenantQuery) loadDNSZoneProviders(ctx context.Context, query *DNSZoneProviderQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *DNSZoneProvider)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
